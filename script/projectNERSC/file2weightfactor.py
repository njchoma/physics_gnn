...
def make_weightfactor_if_not_there(datadir, wf_path, is_used):
    if not os.path.exists(wf_path):
        weight_factors = init_weight_factors(is_used, datadir)
        with open(wf_path, 'wb') as wffile:
            pickle.dump(weight_factors, wffile)
        return weight_factors

    with open(wf_path, 'rb') as wffile:
